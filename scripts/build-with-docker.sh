...
# Container name
CONTAINER_NAME="gnu-cross-build-${OS_LABEL}"
...
docker run -d \
    --name "${CONTAINER_NAME}" \
    -v "${PROJECT_ROOT}:/workspace/gnu-cross" \
    -w /workspace/gnu-cross \
    "${CONTAINER_IMAGE}" \
    tail -f /dev/null
...
echo "Starting build..."
echo "=========================================="
docker exec "${CONTAINER_NAME}" bash -c "cd /workspace/gnu-cross && ./scripts/make ${TARGET}"
